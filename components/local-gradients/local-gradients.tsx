...
export default function LocalGradients() {
  ...
  return (
    <section className="space-y-4">
      <h2 className="text-xl font-bold px-1">My Gradients</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {items.map((gradient, i) => {
          ...
          return (
            <div key={i}>
              <Gradient
                colours={extractHexColors(gradient.controls as string[] | ColorEntry[])}
                name={gradient.name}
              />
              <div className="flex items-center gap-1 mt-1 px-0.5">
                {isEditing ? (
                  ...
                  <div className="flex gap-1 ml-auto">
                    <button onClick={() => startEdit(i)} className="p-1 text-muted-foreground hover:text-foreground rounded" aria-label="Rename gradient">
                      <Pencil className="h-3 w-3" />
                    </button>
                    <button onClick={() => handleDelete(i)} className="p-1 text-muted-foreground hover:text-destructive rounded" aria-label="Delete gradient">
